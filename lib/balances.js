...
// Net position per person, in cents. Positive means the group owes them,
...
export function computeBalances(memberIds, expenses, settlements) {
  ...
  const add = (id, cents) => net.has(id) && net.set(id, net.get(id) + cents);
...
export function simplify(balances) {
  ...
  while (d < debtors.length && c < creditors.length) {
    ...
    transfers.push({ fromUserId: debtors[d].id, toUserId: creditors[c].id, amountCents: amount });
