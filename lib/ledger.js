...
export async function loadLedger(groupId) {
  ...
  const [expenseRows, settlementRows] = await Promise.all([
    sql`
      select e.id, e.payer_id, e.amount_paise, e.description, e.split_type, e.created_at,
             coalesce(
               json_agg(json_build_object('userId', s.discord_user_id, 'amountPaise', s.amount_paise))
    ...
    `,
    sql`
      select id, from_user_id, to_user_id, amount_paise, created_at
      from settlements
  ...
  const expenses = expenseRows.map((e) => ({
    id: int(e.id),
    payerId: e.payer_id,
    amountPaise: int(e.amount_paise),
    description: e.description,
    splitType: e.split_type,
    createdAt: e.created_at,
    shares: e.shares.map((s) => ({ userId: s.userId, amountPaise: int(s.amountPaise) })),
  }));
  ...
  const settlements = settlementRows.map((s) => ({
    ...
    amountPaise: int(s.amount_paise),
