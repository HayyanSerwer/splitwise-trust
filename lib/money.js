// Amounts are integer cents. Parsing happens once, at the edge; nothing
// downstream ever sees a float, because 1/3 of a euro cannot be represented
// in binary floating point and the error compounds across a group's ledger.

const MAX_CENTS = 1_000_000_00; // €1,000,000 per expense — a sane upper bound.
...
export function parseAmount(input) {
  ...
  if (total > MAX_CENTS) throw badRequest('That amount is implausibly large');
  return total;
}

// en-IE grouping: €1,234.56. Swap to 'de-DE' for the 1.234,56 € convention.
export function formatAmount(cents, currency = 'EUR') {
  const sign = cents < 0 ? '-' : '';
...
// Largest-remainder: everyone gets the floor, then the leftover cents go one
